...
async function onTransmitSignal({signal, seed, returnSeed, senderSocketId}, authToken, responseCb) {
  ...
  this.to("convo#" + user.currentConvoId).emit(recipientRole + "/emit-signal", {signal, seed, returnSeed, senderSocketId});
  responseCb("okay");
}
